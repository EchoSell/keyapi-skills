...
```bash
node scripts/keyapi-api.mjs --path /v1/tiktok/... --query '{"example":"value"}' --stdout preview
```
...
```bash
node scripts/keyapi-api.mjs --path /v1/tiktok/... --method POST --image-file ./image.jpg --image-field image_base64
```

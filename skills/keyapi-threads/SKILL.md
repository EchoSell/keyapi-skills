...
```bash
node scripts/keyapi-api.mjs --path /v1/threads/... --query '{"example":"value"}' --stdout preview
```
...
```bash
node scripts/keyapi-api.mjs --path /v1/threads/... --method POST --image-file ./image.jpg --image-field image_base64
```
...
- `references/threads-social-rules.md`

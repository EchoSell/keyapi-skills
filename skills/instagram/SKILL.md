...
This skill turns Instagram KeyAPI HTTP calls into scenario-based workflows for non-technical users.
...
Apply `references/instagram-rules.md` for Instagram-specific entity, identifier, pagination, and output rules.
...
7. Apply `references/instagram-rules.md` before choosing identifiers, pagination fields, freshness mode, and output format.
...
10. Search the latest KeyAPI docs with `node ./search-keyapi-docs.mjs --platform instagram --query "<entity action>"` when endpoint paths, parameters, enums, or pagination are unclear.
...
```bash
node ./search-keyapi-docs.mjs --platform instagram --query "<entity action>"
```

Execute a documented GET endpoint:

```bash
node ./keyapi-api.mjs --path /v1/instagram/... --query '{"example":"value"}'
```

Execute a documented POST endpoint:

```bash
node ./keyapi-api.mjs --path /v1/instagram/... --method POST --body '{"example":"value"}'
```

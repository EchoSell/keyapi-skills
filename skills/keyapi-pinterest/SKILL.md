...
```bash
node scripts/keyapi-api.mjs --path /v1/pinterest/... --query '{"example":"value"}'
```

Execute a large response with preview and local file output:

```bash
node scripts/keyapi-api.mjs --path /v1/pinterest/... --query '{"example":"value"}' --stdout preview
```
...
```bash
node scripts/keyapi-api.mjs --path /v1/pinterest/... --method POST --image-file ./image.jpg --image-field image_base64
```

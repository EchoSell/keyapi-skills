...
```bash
node scripts/keyapi-api.mjs --path /v1/twitter/... --query '{"example":"value"}' --stdout preview
```
...
```bash
node scripts/keyapi-api.mjs --path /v1/twitter/... --method POST --image-file ./image.jpg --image-field image_base64
```
